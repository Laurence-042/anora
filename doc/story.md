...
- **连接判断**：禁用的边**仍然算作连接**，节点在判断是否 ready 时会考虑禁用边连接的入 Port
- **数据传播**：数据**不会**通过禁用的边传播
- **效果**：A→B 结构中，边启用时先 A 后 B；边禁用时只执行 A，B 因为认为有入边但收不到数据而永远不会 ready
- **用途**：调试时临时阻断数据流，同时保持图的逻辑结构
